...
# Complete master script to write out full process_data.py with canonical role names and 2.2.2 standard for all 28 Level 3 processes.

file_path = "/Users/billy/SAPB1_Sim/process_data.py"
        ...
        "processes": [
            {
                ...
                "steps": [
                    ...
                    ("2.1.1", "2.1.1.6", "[Finance] Post Direct AR Tax Invoice (OINV) @ FDMSYD → [Customer Dispatch & Revenue Recognition]", "OINV / INV1", "old_b1.db", "FDMSYD",
                     "Customer Custody\\n[FDMSYD OnHand = 0]",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.1.2", "2.1.2.6", "[Finance] Post Direct Export AR Tax Invoice (OINV) @ FDMSYD → [Cross-Border Dispatch & Zero-Rated Revenue Recognition]", "OINV / INV1", "old_b1.db", "FDMSYD",
                     "Customer Custody\\n[FDMSYD OnHand = 0]",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.1.3", "2.1.3.11", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Dispatch & Channel Revenue Recognition]", "OINV / INV1", "old_b1.db", "UKWYF",
        ...
        "processes": [
            {
                ...
                "steps": [
                    ...
                    ("2.2.1", "2.2.1.12", "[Finance] Post Direct AR Tax Invoice (OINV) @ FDMSYD → [Customer Dispatch & Revenue Recognition]", "OINV / INV1", "old_b1.db", "FDMSYD / BDLMEL",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.2", "2.2.2.12", "[Finance] Post Direct Export AR Tax Invoice (OINV) @ FDMSYD → [Cross-Border Dispatch & Zero-Rated Revenue Recognition]", "OINV / INV1", "old_b1.db", "FDMSYD / BDLMEL",
                     "Customer Custody\\n[OnHand = 0]",
                     "• Created directly from ORDR (bypasses standalone ODLN)\\n• Realized COGS: 100 x $522.00 = $52,200.00 AUD\\n• Zero-rated export revenue (GST-free) = $92,000.00 AUD",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.3", "2.2.3.15", "[Finance] Post Direct AR Tax Invoice (OINV) @ NZNTH → [Domestic NZ Customer Dispatch & Revenue Recognition]", "OINV / INV1", "new_b1.db", "NZNTH",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.4", "2.2.4.13", "[Finance] Post Direct AR Tax Invoice (OINV) @ FDMSYD → [Customer Dispatch & Revenue Recognition]", "OINV / INV1", "old_b1.db", "FDMSYD",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.5", "2.2.5.9", "[Finance] Post Direct AR Tax Invoice (OINV) @ NZNTH → [Domestic Customer Dispatch & Revenue Recognition]", "OINV / INV1", "new_b1.db", "NZNTH",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.6", "2.2.6.9", "[Finance] Post Direct AR Tax Invoice (OINV) @ FDMSYD → [Customer Dispatch & Revenue Recognition]", "OINV / INV1", "old_b1.db", "FDMSYD",
                     "Customer Custody\\n[OnHand = 0]", "Direct from ORDR (bypasses standalone ODLN); COGS = $54,500 AUD, Revenue = $85,000 + GST", "Dr 110010 ($93,500) / Dr 500010 ($54,500) / Cr 400010 ($85,000) / Cr 200020 ($8,500) / Cr 100010 ($54,500)")
                ]
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.7", "2.2.7.13", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Channel Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.8", "2.2.8.13", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Channel Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.9", "2.2.9.12", "[Sales] Book Wholesale Sales Order (ORDR) @ UKWYF → [Wayfair Channel Allocation]", "ORDR / RDR1", "new_b1.db", "UKWYF",
                     "UKWYF [Reserved = +100]", "Wholesale agreement @ $750.00 AUD", "Sales Order Commitment"),
                    ("2.2.9", "2.2.9.13", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Channel Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
                     "Wayfair Custody\\n[OnHand = 0]", "Direct from ORDR (bypasses standalone ODLN); COGS = $56,000, Revenue = $75,000", "Dr 110010 ($75,000) / Dr 500010 ($56,000) / Cr 400010 ($75,000) / Cr 100040 ($56,000)")
                ]
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.10", "2.2.10.16", "[Sales] Book Wholesale Sales Order (ORDR) @ UKWYF → [Wayfair Channel Allocation]", "ORDR / RDR1", "new_b1.db", "UKWYF",
                     "UKWYF [Reserved = +100]", "Wholesale agreement @ $750.00 AUD", "Sales Order Commitment"),
                    ("2.2.10", "2.2.10.17", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Channel Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
                     "Wayfair Custody\\n[OnHand = 0]", "Direct from ORDR (bypasses standalone ODLN); COGS = $56,000, Revenue = $75,000", "Dr 110010 ($75,000) / Dr 500010 ($56,000) / Cr 400010 ($75,000) / Cr 100040 ($56,000)")
                ]
            },
            {
                ...
                "steps": [
                    ...
                    ("2.2.11", "2.2.11.16", "[Sales] Book Wholesale Sales Order (ORDR) @ UKWYF → [Wayfair Channel Allocation]", "ORDR / RDR1", "new_b1.db", "UKWYF",
                     "UKWYF [Reserved = +100]", "Wholesale agreement @ $750.00 AUD", "Sales Order Commitment"),
                    ("2.2.11", "2.2.11.17", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Channel Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
                     "Wayfair Custody\\n[OnHand = 0]", "Direct from ORDR (bypasses standalone ODLN); COGS = $56,000, Revenue = $75,000", "Dr 110010 ($75,000) / Dr 500010 ($56,000) / Cr 400010 ($75,000) / Cr 100040 ($56,000)")
                ]
            }
        ]
        ...
        "processes": [
            {
                ...
                "steps": [
                    ...
                    ("2.3.1", "2.3.1.5", "[Sales] Book Domestic NZ Customer Sales Order (ORDR) @ NZNTH → [Customer Stock Allocation]", "ORDR / RDR1", "new_b1.db", "NZNTH",
                     "NZNTH [Reserved = +100]", "Domestic sales order booking", "Sales Order Commitment"),
                    ("2.3.1", "2.3.1.6", "[Finance] Post Direct AR Tax Invoice (OINV) @ NZNTH → [Domestic Customer Dispatch & Revenue Recognition]", "OINV / INV1", "new_b1.db", "NZNTH",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.3.2", "2.3.2.11", "[Finance] Post Direct AR Tax Invoice (OINV) @ NZSTH → [Customer Dispatch & Revenue Recognition]", "OINV / INV1", "new_b1.db", "NZSTH",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.3.3", "2.3.3.5", "[Sales] Book Wayfair EDI Wholesale Order (ORDR) @ UKWYF → [Channel Stock Allocation]", "ORDR / RDR1", "new_b1.db", "UKWYF",
                     "UKWYF [Reserved = +100]", "Wholesale order booking @ $750.00 AUD", "Sales Order Commitment"),
                    ("2.3.3", "2.3.3.6", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.3.4", "2.3.4.8", "[Sales] Book Wayfair Wholesale Order (ORDR) @ UKWYF → [Finished Goods Allocation]", "ORDR / RDR1", "new_b1.db", "UKWYF",
                     "UKWYF [Reserved = +100]", "Wholesale agreement @ $750.00 AUD", "Sales Order Commitment"),
                    ("2.3.4", "2.3.4.9", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
                     "Wayfair Custody\\n[OnHand = 0]", "Direct from ORDR (bypasses standalone ODLN); COGS = $56,000 AUD, Revenue = $75,000", "Dr 110010 (AR) $75,000\\nDr 500010 (COGS UK) $56,000\\nCr 400010 (Revenue) $75,000\\nCr 100040 (UKWYF) $56,000")
                ]
            },
            {
                ...
                "steps": [
                    ...
                    ("2.3.5", "2.3.5.8", "[Sales] Book Wholesale Sales Order (ORDR) @ UKWYF → [Wayfair Channel Allocation]", "ORDR / RDR1", "new_b1.db", "UKWYF",
                     "UKWYF [Reserved = +100]", "Wholesale agreement @ $750.00 AUD", "Sales Order Commitment"),
                    ("2.3.5", "2.3.5.9", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Channel Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
                     "Wayfair Custody\\n[OnHand = 0]", "Direct from ORDR (bypasses standalone ODLN); COGS = $56,000, Revenue = $75,000", "Dr 110010 ($75,000) / Dr 500010 ($56,000) / Cr 400010 ($75,000) / Cr 100040 ($56,000)")
                ]
            },
            {
                ...
                "steps": [
                    ...
                    ("2.3.6", "2.3.6.12", "[Sales] Book Wholesale Sales Order (ORDR) @ UKWYF → [Wayfair Channel Allocation]", "ORDR / RDR1", "new_b1.db", "UKWYF",
                     "UKWYF [Reserved = +100]", "Wholesale agreement @ $750.00 AUD", "Sales Order Commitment"),
                    ("2.3.6", "2.3.6.13", "[Finance] Post Direct Wholesale AR Invoice (OINV) @ UKWYF → [Wayfair Channel Revenue Recognition]", "OINV / INV1", "new_b1.db", "UKWYF",
                     "Wayfair Custody\\n[OnHand = 0]", "Direct from ORDR (bypasses standalone ODLN); COGS = $56,000, Revenue = $75,000", "Dr 110010 ($75,000) / Dr 500010 ($56,000) / Cr 400010 ($75,000) / Cr 100040 ($56,000)")
                ]
            },
            {
                ...
                "steps": [
                    ...
                    ("2.3.7", "2.3.7.5", "[Finance] Post Direct AR Tax Invoice (OINV) @ ICCNGB → [Direct Air Dispatch & Revenue Recognition]", "OINV / INV1", "new_b1.db", "ICCNGB",
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.3.8", "2.3.8.4", "[Sales] Book D2C Consumer Sales Order (ORDR) @ ICCNGB → [NZ Consumer Stock Allocation]", "ORDR / RDR1", "new_b1.db", "ICCNGB",
                     "ICCNGB [Reserved = +100]", "Consumer sales order: Unit Price = $950.00 AUD", "Sales Order Commitment"),
                    ("2.3.8", "2.3.8.5", "[Finance] Post Direct AR Tax Invoice (OINV) @ ICCNGB → [Direct Air Dispatch & Revenue Recognition]", "OINV / INV1", "new_b1.db", "ICCNGB",
                     "Customer Custody\\n[ICCNGB OnHand = 0]", "Direct from ORDR (bypasses standalone ODLN); COGS = $58,000 AUD, Revenue = $95,000", "Dr 110010 ($95,000)\\nDr 500010 (COGS D2C) $58,000\\nCr 400010 (Revenue) $95,000\\nCr 100040 (ICCNGB) $58,000")
                ]
            },
            {
                ...
                "steps": [
                    ...
                    ("2.3.9", "2.3.9.5", "[Finance] Post Direct AR Tax Invoice (OINV) @ ICCNGB → [Direct Air Dispatch & Revenue Recognition]", "OINV / INV1", "new_b1.db", "ICCNGB",
        ...
        "processes": [
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("2.4.3", "2.4.3.4", "[Finance] Post Surplus Inward GRPO & Supplementary Invoice (OPDN/OINV) @ FDMSYD & ICCNGB → [Surplus Settlement]", "OPDN (old_b1) + OINV (new_b1)", "old_b1 & new_b1", "FDMSYD & ICCNGB",
...
print("Successfully wrote full process_data.py with canonical role names and 2.2.2 standard across all 28 processes.")
